...
class SASFDataset(Dataset):
    def __init__(self, robot_paths, robot_names, leg2idx, max_sample_size):
    ...
    def __getitem__(self, index):
        ...
        sample_idx = np.random.choice(robot_dynamic.shape[1], sample_size, replace=True)
        sampled_robot_dynamics = robot_dynamic[:, sample_idx].reshape(18, -1)
    ...
    def load_data(self, robot_paths):
        for robot_name in tqdm(self.robot_names, desc="Loading Data"):
            dynamic_data = np.load(robot_paths[robot_name]).astype(dtype=np.float32)
    ...
    def collate(self, batch):
        ...
        for b in sorted_batch:
            ...
            dynamics = dynamics.T
            padded_dynamics = np.zeros((self.max_sample_size * 16, 18), dtype=np.float32)
            ...
            length_list.append(torch.tensor(dynamics.shape[1]))
...
def main():
    ...
    use_wandb = True
    if use_wandb:
        wandb.init(project="sig_size_exp", entity="robotics")
        config = wandb.config
    ...
    train_dataset = SASFDataset(robot_paths, train_robot_names, leg2idx, max_sample_size=config.max_sample_size)
    valid_dataset = SASFDataset(robot_paths, valid_robot_names, leg2idx, max_sample_size=config.max_sample_size)
    ...
    # Setup model
    model = PredConf(do=config.dropout, mlp_hidden_dim=config.mlp_hidden_dim)
